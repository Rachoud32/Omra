...
export class SearchFormComponent implements OnInit {
  ...
  deletePassengermap() {
    if (this.passengersmap.length > 1) {
      ...
      this.numChildren = this.numChildren - this.passengersmap.at(this.passengersmap.length - 1).get('children')?.value.length
  ...
  plusadultsmap(i: any) {
    if (this.passengersmap.at(i).get('adultsmap')?.value < 5) {
      this.passengersmap.at(i).get('adultsmap')?.setValue(this.passengersmap.at(i).get('adultsmap')?.value + 1)
      this.numAdults += 1
    }
  }
  ...
  minusadultsmap(i: any) {
    if (this.passengersmap.at(i).get('adultsmap')?.value > 1) {
      this.passengersmap.at(i).get('adultsmap')?.setValue(this.passengersmap.at(i).get('adultsmap')?.value - 1)
      this.numAdults -= 1
    }
  }
